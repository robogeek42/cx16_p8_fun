...
main {
	...
	; bitmap 320x240 8bpp = 76k = $1:2C00 - starting at $0:0000
	;
	; bitmap 8bpp $00000 -> $12BFF 
	; map0        $13000 -> $137FF : 32x32 * 2 bytes is 2k
	; sprites     $14000 -> $14FFF : 4k (bob takes 3k (24 sprites), + 4 cursor sprites)
	; tiles (4bpp)       $15000 -> $15FFF : 4k - terrain only
	; tiles (8bpp)       $16000 -> $1EFFF : 36k
	...
	const uword tile1BaseAddr = $6000
...
;============================================================
	sub setup_screen() {
		...
		; Clear the screen bitmap - cheat: go into graphics mode, clear then setup the screen.
		; as we are using the default bitmap location (0) then this works
		cx16.GRAPH_init(0)
        ...
        ; enable 320*240  4bpp tile-mode in Layer 0 and 8bpp bitmap-mode in Layer 1
        cx16.VERA_CTRL=0
        ...
        cx16.VERA_L1_CONFIG = %00000111 ; color depth (11) = 256 cols, bitmap mode on
        cx16.VERA_L1_MAPBASE = 0
        cx16.VERA_L1_TILEBASE = 0 | %00 ; bitmap screen size = 320x240 (lores)
	}
...
;			}
	...
	sub do_scroll(ubyte dir, ubyte bob_speed)
	{
		...
		load_bmp_offset(screen_offset_px >> 4, screen_offset_py >> 4)
		show_cursor(0)
	}
...
;============================================================
	...
	const ubyte PAL_terrain  = 3
	const ubyte PAL_bob      = 4
	sub load_palette()
	{
		; load the unified palette
		;   0   : CX16 default colours
		;   1-2 : Features, Belts, Machines, Items (8bpp)
		;   3   : Terrain (4bpp)
		;   4   : FAC Bob (4bpp)
...
ubyte tiles_count
	...
	sub load_tiles_8bpp() {
		...
		ubyte n
		bool ret
		uword tbaddr = tile1BaseAddr
		ubyte bank = tile1BaseBank
		str filename = " "*20
		ubyte id = 0
...
;		}
	...
	sub load_map_offset(uword src_col, uword src_row) {
		...
		load_bmp_offset(src_col, src_row)
	...
	sub draw_bitmap_16x16_at_tilexy(ubyte id, ubyte tx, ubyte ty) {
		uword src_vram_addr = tile1BaseAddr + id * 256
		ubyte dst_vram_bank = 0
		uword dst_vram_addr = 0

		uword dst_tile = 0
		uword px = tx as uword
		uword py = ty as uword
		if py*20+px>255 dst_vram_bank = 1
		px = px << 4
		py = py << 4
		dst_vram_addr = py * 320
		dst_vram_addr += px

		cx16.VERA_CTRL = 0
		cx16.VERA_ADDR_L = lsb(src_vram_addr)
		cx16.VERA_ADDR_M = msb(src_vram_addr)
		cx16.VERA_ADDR_H = 1 | %00010000      ; increment 1

		ubyte row, col
		for row in 0 to 15 
		{
			cx16.VERA_CTRL = 1
			cx16.VERA_ADDR_L = lsb(dst_vram_addr)
			cx16.VERA_ADDR_M = msb(dst_vram_addr)
			cx16.VERA_ADDR_H = dst_vram_bank | %00010000      ; increment 1
			...
			if (dst_vram_addr+320) < dst_vram_addr dst_vram_bank += 1
			dst_vram_addr += 320
		}
		cx16.VERA_CTRL = 0
	...
	sub load_bmp_offset(uword src_col, uword src_row) {
		; load the screen portion of the bitmap layer with features and machines
		ubyte x, y
		ubyte paloff
		...
		; features in map0 
		for y in 0 to 14 {
			cpuoffset = (src_row+y) * map_width_tilesw + src_col
			cpuptr = bankedRAM + (cpuoffset % $2000)
			for x in 0 to 19 {
				cx16.rambank( lsb((cpuoffset / $2000) + map0rambank) )
				...
				if feat > 0 {
					draw_bitmap_16x16_at_tilexy(feat, x, y)
				}
